...
export default class JSONCache<T = any> implements IJSONCache<T> {
  ...
  constructor(
    ...
    this.options.prefix =
      typeof options.prefix === 'string' ? options.prefix : 'jc:';
  ...
  public async set(
    ...
    const commands: IMultiCommands = await this.getKeysToBeRemoved(
      key,
      flattened,
    );
      ...
      result = fields.reduce(
        (res, field) => {
          if (field in data) {
            ...
            (dataKeys || (dataKeys = Object.keys(data))).forEach(
              (flattenedKey) => {
  ...
  private async getKeysToBeRemoved(
    key: string,
    flattened: IResult,
  ): Promise<IMultiCommands> {
    const commands: IMultiCommands = [];
