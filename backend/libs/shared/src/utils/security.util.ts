...
export class SecurityUtil {
  ...
  static parse(key: any, type: 'PRIVATE' | 'PUBLIC'): string {
    ...
    const beginLine = START_END.filter(
      (o) => o.includes('BEGIN') && o.includes(type),
    )[0];
    const endLine = START_END.filter(
      (o) => o.includes('END') && o.includes(type),
    )[0];
    return `${beginLine}\n${temp}\n${endLine}`;
  }
  //
    ...
    try {
      ...
      if (!privateKey) {
        throw {
          ...
          message: `Configuration value for 'fcmPrivateKey' not found`,
        };
      }
      ...
      if (!data) {
        throw {
          Code: HttpStatus.UNAUTHORIZED,
          message: `Request requires valid request body`,
        };
      }
      const signature = sign.sign(
        SecurityUtil.parse(privateKey, 'PRIVATE'),
        'base64',
      );
      return signature;
    } catch (err) {
  ...
  static verify(
    ...
    try {
      ...
      const verified = verify.verify(
        SecurityUtil.parse(publicKey, 'PUBLIC'),
        signature,
        'base64',
      );
