import { UserApiAccessBuilder } from '@app/auth/dtos';
...
export class ClientAccessService {
  ...
  private async getClientAccess(
    options: Partial<{ id: number; userId: number }>,
  ): Promise<UserAccessEntity | undefined> {
  ...
  public async getClientAccessById(
    options: Partial<{ id: number; userId: number }>,
  ): Promise<DetailResponse<UserAccessEntity> | undefined> {
    const data = await this.getClientAccess(options);
    return new DetailResponse(data);
  }

  public async getClientAccesses(options: CleantAccessPageOptionsDto): Promise<PageDto<UserAccessEntity> | undefined> {
    const qb = this.ds.getRepository(UserAccessEntity).createQueryBuilder('ua');
    qb.select(['ua.id', 'ua.apiClientId', 'ua.clientName', 'ua.allowedUrls', 'ua.status', 'ua.userId', 'ua.createdAt']);
    ...
    qb.orderBy(`ua.${options.sort || 'createdAt'}`, options.order);
  ...
  public async createClientAccess(
    dto: CreateClientAccessDto,
  ): Promise<DetailResponse<ClientAccessWithCredentials> | undefined> {
