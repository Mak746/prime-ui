...
import { NotificationConfigDto } from '@app/global-config/dto';
...
export class UserNotificationService {
  ...
  private _closing = () => `\nThank you for choosing WeCare.et!`;
  ...
  private async _getNotificationConfig() {
    const config = (await this.globalConfigService.getConfigByKey<NotificationConfigDto>(
      'notification',
  ...
  public async sendAuthSMS(data: IAccountSMS) {
    ...
    console.log(
      'notificationConfig?.isSmsNotificationInSimulation:',
      notificationConfig?.isSmsNotificationInSimulation,
    );
  ...
  public async sendWelcomeEmail(data: UserEntity, otpCode: number, password: string) {
    if (data.email) {
      const loginUrl = data?.realm === 'ADMIN' ? `https://manage.${this.serverDns}` : `https://${this.serverDns}`;
      const payload: IEMail<IWelcomeUser> = {
        to: data.email,
        userId: data.id,
        context: { loginUrl, fullName: data.username, otpCode, password },
      };
  ...
  public async sendResendOTPEmail(data: UserEntity, otpCode: number, ua: AccessUserAgent) {
    if (data.email) {
      const supportUrl = `info@wecare.com`;
