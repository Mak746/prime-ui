...
export class Utils {
  ...
  public static defaultHashVerifier(token: string, dataCheckString: string, hash: string, callback: HashVerifierCallbackFunction): void | Promise<void> {
    ...
    const secret = createHmac('sha256', 'WebAppData').update(token).digest('hex');
