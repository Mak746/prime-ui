...
export class GlobalConfigService {
  ...
  public async initConfig(): Promise<any | undefined> {
    ...
    try {
      ...
      Object.keys(CONFIG_INIT).forEach((key) => {
        const group = Object.keys(CONFIG_INIT[key]).map((configKey) => ({
          category: key,
          key: configKey,
          value: CONFIG_INIT[key][configKey],
        }));
        data.push(...group);
      });
  ...
  public async getGlobalConfigById(key: string): Promise<GlobalConfigEntity | undefined> {
    return this.ds
      .createQueryBuilder(GlobalConfigEntity, 'globalConfig')
      .andWhere('globalConfig.key = :key', { key })
      .getOne();
  }

  public async getGlobalConfigs(options: GlobalConfigPageOptionsDto): Promise<PageDto<GlobalConfigEntity> | undefined> {
    const qb = this.ds.createQueryBuilder(GlobalConfigEntity, 'globalConfig');
    const [globalConfigs, globalConfigsCount] = await qb
      .skip((options.page - 1) * options.take)
      .take(options.take)
      .orderBy(`globalConfig.${options.sort || 'updatedAt'}`, options.order)
      .getManyAndCount();
  ...
  public async updateGlobalConfig(
    ...
    key: string,
  ): Promise<GlobalConfigEntity | undefined> {
  ...
  public async getConfigs(): Promise<SystemConfigDto | undefined> {
    try {
      ...
      const app = await this.getConfigByKey<AppConfigDto>('app');

      return { auth, app, general, notification };
    } catch (error) {
      Logger.error({
  ...
  public async saveConfig<T>(key: string, globalConfigDto: GlobalConfigAdapter): Promise<any | undefined> {
    ...
    try {
      ...
      await queryRunner.manager.upsert(GlobalConfigEntity, config, {
        conflictPaths: ['key'],
      });
