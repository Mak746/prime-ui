...
export class AppConfigDto implements GlobalConfigAdapter {
  getConfigs(): GlobalConfigEntity {
    return new GlobalConfigEntity({ key: 'app', value: JSON.stringify(this) });
  }

  @IsBoolean()
  @Type(() => Boolean)
  automaticallyCallUsers: boolean;

  constructor(partial: Partial<AppConfigDto>) {
    return Object.assign(this, partial);
  }
}
...
const auth = new AuthConfigDto({
  accessTokenLifespan: EXP_ACC,
  refreshTokenLifespan: EXP_REF,
});

const app = new AppConfigDto({
  automaticallyCallUsers: true,
});
