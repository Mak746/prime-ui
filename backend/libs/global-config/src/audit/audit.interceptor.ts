...
export class AuditInterceptor implements NestInterceptor {
  ...
  private async _logStaffActivity(payload: CreateActivityLogDto): Promise<void> {
    ...
    try {
      ...
      this.logger.error({
        message: 'ERROR @ _logStaffActivity',
        meta: payload,
        stack: error,
        context: this.ctxPrefix,
      });
    } finally {
    ...
    return next.handle().pipe(
      tap(async (data) => {
        ...
        if (config && config.enableApiRequestResponseLog) {
          ...
          const logTitle = this._reflector.getAllAndOverride<string>('activityTitle', [
            context.getHandler(),
            context.getClass(),
          ]);
